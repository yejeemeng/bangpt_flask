...
InfoPost_bp = Blueprint('info_post', __name__)
#--------------------------------------------------------------------------------------------------------------------#


@InfoPost_bp.route('/post', methods=['POST'])
def post():
    ...
    try:
        ...
        result = InfoPosts.insert_one(board)
...
@InfoPost_bp.route('/', methods=['GET'])
def get_info_posts():
    cursor = InfoPosts.find({})
    posts = list(cursor)
        
    return dumps(posts)


@InfoPost_bp.route('/load_board_title/<userid>', methods=['POST']) # commu~Activity에 list로 넘겨줄 값들
...
@InfoPost_bp.route('/load_board_detail', methods=['POST']) # WriteviewActivity와 연결
def load_board_detail():
    board_seq = request.form['board_seq']
    board = InfoPosts.find_one({'_id': ObjectId(board_seq)})
...
@InfoPost_bp.route('/load_cmt', methods=['POST']) # 댓글을 불러옴
def load_cmt():
...
@InfoPost_bp.route('/reg_cmt', methods=['POST'])  # 댓글 작성
def reg_comment():
